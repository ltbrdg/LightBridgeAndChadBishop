function ShowCase($wrapper){
	...
	this.filterClick = function(event){
		...
		event.data.$viewMore.trigger('click',event.data);
	};
	...
	this.expandList = function(event,obj){		
		...
		openWrapper = function(){
		    event.data = (!event.data)? obj : event.data;
	...
	this.init = function(){
		...
		this.$viewMore.on('click',this,this.expandList);
		// item click
